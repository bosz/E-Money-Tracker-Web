  ...
  const tableData = transactions.map((transaction, index) => {
    return (
      <tr key={index} className=" align-items-center">
        ...
        <td>
          {transaction.status === 'Validated' && <span className="text-success">{transaction.status}</span>}
          {transaction.status === 'New' && <span className="text-primary">{transaction.status}</span>}
          {transaction.status === 'Pending' && <span className="text-warning">{transaction.status}</span>}
        </td>
        {transaction.status !== 'Validated' ? (
          ...
          <td>
              <button className="btn p-0 text-secondary btn-md">
                <i class="fal text-success fa-check-double fa-2x"></i>
              </button>
          </td>
        )}
      </tr>
